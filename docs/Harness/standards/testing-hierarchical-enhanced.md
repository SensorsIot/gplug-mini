...
<!--
Revision note:
- Introduces hierarchical atomic/workflow test cases.
- Defines the complete single-file plan schema and generation algorithm.
- Adds reusable actions, TDDs, required workflows and release gates.
- Updates workbench, meter, OTA and field procedure facts.
-->
...
metadata:
  ...
  migration:
    existing_atomic_tests: <count>
    added_workflows: <count>
    old_kind_moved_to: scenario
    new_kind_values: [atomic, workflow]
```

When converting the previous flat schema:
...
The first seven execute the FSD acceptance scenarios. The additional five close
product-goal and release seams that isolated requirements do not prove as a
continuous journey.
...
The gPlug-mini plan requires at least:

| ID | Subject | Needed by |
|---|---|---|
| `TDD-001` | exact simulated-day sequence and oracle | `WF-002` |
| `TDD-002` | maximum OTA duration and validation timeout | `WF-004`, `WF-005` |
| `TDD-003` | real-meter comparison tolerance and timing window | `WF-006`, `WF-007` |
| `TDD-004` | production-image no-publication observation window | `WF-010` |
| `TDD-005` | physical and repeatable reconfiguration test method | `WF-011` |

An open decision makes an affected case `not done` or `blocked`; it does not
permit the generator to invent a value.
...
8. **Create product and release workflows.**
   ...
   reconfiguration and watchdog recovery.
...
**Mode 2 is the stable acceptance signal.** It repeatedly delivers complete
144-byte cycles with power, energy and meter identity. Use it for standard
journeys and for workflow handoffs.

**Mode 3 is not a valid single-cycle acceptance oracle on the current rig.** The
long telegram is frequently truncated. A case using it must span several cycles,
require at least one complete cycle, and record the capability limitation rather
than blaming a truncated burst on the firmware.
