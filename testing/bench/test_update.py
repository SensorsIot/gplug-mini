...
from conftest import BENCH_AP_GATEWAY

PROJECT = "gplug-mini"


def image_url(wb_url, filename):
    """The URL as the board sees it — the Pi is the AP gateway on the bench network,
    not the LAN address this test client uses."""
    return f"http://{BENCH_AP_GATEWAY}:8080/firmware/{PROJECT}/{filename}"
...
def test_ts062_neg_a_malformed_command_is_refused_and_said_so(dut, broker_host, relay, dut_mac):
    ...
    mac = dut_mac
    dut.drain()
    publish_ota(broker_host, mac, f"{BENCH_AP_GATEWAY}/app.bin")   # no scheme
...
def test_ts084_the_reset_reason_is_reported(dut):
    ...
    # the earliest line any monitor sees is around t=2650 ms, by which point the
    # boot report is long gone. The firmware repeats it once the network is up,
